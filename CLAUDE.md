...
| `sitemap.xml`, `robots.txt` | SEO files | **Generated** (script 7) |
...
## SEO conventions (added 2026-06-12)

- Every page has: a descriptive `<title>` ending in `| Afrin Archive`, a meta
  description, a `rel="canonical"` URL on `https://www.afrinarchive.com` (percent-encoded
  path — filenames contain Kurdish characters/apostrophes/spaces), and Open Graph +
  twitter:card tags. Generated pages get these from the templates in scripts 0 and 1;
  hand-made pages carry them manually — copy the pattern from an existing page.
- Village pages additionally embed **JSON-LD `Place`** markup (name, alternateName from
  the note's aliases, geo coordinates, nahiya) — built in `build_seo_head()` in script 1.
- `sitemap.xml` and `robots.txt` are regenerated by `7_make_seo.py` on every site update.
  New public folders must be added to its `PUBLIC_FOLDERS` allowlist.
